import"./modulepreload-polyfill-Dezn_h7o.js";var e=await fetch(`/api/v1/b20-inventory-agent`,{headers:{accept:`application/json`}});if(!e.ok)throw Error(`B20 inventory candidate unavailable`);var t=await e.json(),n=(e,t=10,n=8)=>e&&e.length>t+n?`${e.slice(0,t)}...${e.slice(-n)}`:e,r=t.six_lanes,i=(e,t,n,r)=>`<div class="stage"><span>${String(e).padStart(2,`0`)}</span><div><strong>${t}</strong><small>${n}</small></div><em>${r}</em></div>`,a=e=>`<li><span class="${e.passed?`pass`:`fail`}"></span><strong>${e.id.replaceAll(`_`,` `)}</strong><code>${e.observed}</code></li>`;document.querySelector(`#app`).innerHTML=`
  <header class="topbar"><a href="/enterprise-os.html">CATVERSE Base Lab OS</a><span>Deployment-ready candidate · no token deployed</span></header>
  <main>
    ...
    <section class="activation strip"><div><span>B20 Asset</span><strong>${t.activation.asset_active?`Active`:`Inactive`}</strong></div><div><span>Production target</span><strong>${t.activation.target}</strong></div><div><span>Testnet rehearsal</span><strong>${t.deployment_gate.current_testnet_rehearsal.network.name}</strong></div><div><span>Factory</span><code>${n(t.token.factory)}</code></div><div><span>Deployed</span><strong>No</strong></div></section>
    ...
    <section class="next"><div><span>VERIFIED TESTNET RESULT</span><h2>One Base Sepolia CATBOX createB20 completed</h2><p>The current EOA route ${n(t.deployment_gate.current_testnet_rehearsal.deployer)} created ${n(t.deployment_gate.testnet_creation.token_address)} through the official B20 Factory in ${n(t.deployment_gate.testnet_creation.tx_hash)}. It used zero ETH value and preserved zero supply, no mint and no activation. Mainnet remains unconfigured.</p></div><dl><dt>Receipt</dt><dd>${t.deployment_gate.testnet_creation.receipt_status}</dd><dt>Mint / activation</dt><dd>Not bundled</dd><dt>ERP write</dt><dd>None</dd><dt>Mainnet</dt><dd>Not started</dd></dl></section>
  </main>
