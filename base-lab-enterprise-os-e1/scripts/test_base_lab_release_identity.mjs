...
test("review pack combines Base lanes without adding a write surface", () => {
  ...
  const smartWalletEvidence = {schema_version:"1",publication_unit_id:"connection",product:"CATVERSE",status:"confirmed",network:"Base",smart_wallet:"0xparent",contract:{},confirmed_business_record:{},confirmed_base_app_connection_record:{transaction_hash:"0xconnection",replay_state:"locked_after_confirmed_receipt"},publication_controls:{},negative_controls:[],material_fingerprint_sha256:"0xconnection"};
  const pack = buildReviewPack({xerp01:lane,inventory:lane,asset:lane,catalog,lifecycle,b20InventoryAgent,smartWalletEvidence,release:{commit:"abc"}});
  ...
  assert.equal(pack.lanes.base_app_connection.confirmed_base_app_connection_record.transaction_hash,"0xconnection");
...
test("review pack serves each lane and denies writes", async () => {
  ...
  assert.equal(pack.lanes.base_app_connection.confirmed_base_app_connection_record.transaction_hash, "0x4e159f5ef8be1aa265e143d3e8747bf8089784cd599c4f2a9cc3ae3ece09a664");
  assert.equal(pack.lanes.b20_inventory.token.symbol, "CATBOX");
